...
Downloads raw video → Audio mastering → LUT color grade → Compress to web 4K →
...
LOUDNESS_TARGET = -16  # LUFS (podcast/YouTube standard)
TRUE_PEAK = -1  # dBTP
HIGHPASS_FREQ = 80  # Hz

# Video compression
VIDEO_BITRATE = "18M"  # 18 Mbps for 4K
AUDIO_BITRATE = "192k"
...
# Anamorphic lens detection (Sirui 50mm = 1.33x squeeze)
...
def detect_anamorphic(info: dict) -> bool:
    """Detect if footage needs anamorphic desqueeze"""
    for stream in info.get("streams", []):
        if stream.get("codec_type") == "video":
            w = int(stream.get("width", 0))
            h = int(stream.get("height", 0))
            # Anamorphic 1.33x on 16:9 sensor produces ~1.33:1 aspect stored
            # Standard 16:9 = 1.78, anamorphic squeeze would store as ~1.33
            aspect = w / h if h else 0
            if 1.2 < aspect < 1.45:  # likely squeezed
                return True
    return False
...
def build_ffmpeg_command(input_path: Path, output_path: Path, lut_path: Path,
    ...
    # Anamorphic desqueeze (1.33x horizontal)
    if anamorphic:
...
def process_video(b2_path: str, lut_name: str = "default"):
    ...
    # Resolve LUT
    lut_file = CAMERA_LUTS.get(lut_name, CAMERA_LUTS["default"])
    ...
    anamorphic = detect_anamorphic(info)
    if anamorphic:
        print(f"  ⚠ Anamorphic footage detected — will desqueeze 1.33x")
    for stream in info.get("streams", []):
        if stream.get("codec_type") == "video":
    ...
    # Step 3: Process (audio + LUT + compress)
    print(f"  [3/7] Processing: audio master + LUT + compress...")
    ...
    output = {
        ...
        "processing": {
            ...
            "audio_loudness_target": f"{LOUDNESS_TARGET} LUFS",
            "video_codec": "H.264",
...
def main():
    ...
    target = sys.argv[1] if len(sys.argv) > 1 else "Futuro MMXXV/raw/card-1/Day 1"
    ...
    print(f"LUT: {LUT_DIR / CAMERA_LUTS['default']}")
    print()
