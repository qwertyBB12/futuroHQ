/**
 * Populate ledgerPerson records from user-provided data file.
 *
 * Usage:
 *   npx tsx scripts/populate-ledger-persons.ts [--dry-run]
 *
 * Reads: scripts/data/ledger-persons-data.json
...
async function run() {
  const dataPath = resolve(__dirname, 'data/ledger-persons-data.json')
  let records: LedgerPersonRecord[]

  try {
    records = JSON.parse(readFileSync(dataPath, 'utf-8'))
  } catch {
    console.error(`ERROR: Could not read ${dataPath}`)
    console.error('Create scripts/data/ledger-persons-data.json with an array of records keyed by _id.')
    console.error('See scripts/data-templates/ledger-persons-data.example.json for the expected format.')
    process.exit(1)
  }

  console.log(`Loaded ${records.length} ledgerPerson records from data file`)
