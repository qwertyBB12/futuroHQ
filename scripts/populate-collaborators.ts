/**
 * Populate collaborator records from user-provided data file.
 *
 * Usage:
 *   npx tsx scripts/populate-collaborators.ts [--dry-run]
 *
 * Reads: scripts/data/collaborators-data.json
...
async function run() {
  const dataPath = resolve(__dirname, 'data/collaborators-data.json')
  let records: CollaboratorRecord[]

  try {
    records = JSON.parse(readFileSync(dataPath, 'utf-8'))
  } catch {
    console.error(`ERROR: Could not read ${dataPath}`)
    console.error('Create scripts/data/collaborators-data.json with an array of records keyed by _id.')
    console.error('See scripts/data-templates/collaborators-data.example.json for the expected format.')
    process.exit(1)
  }

  console.log(`Loaded ${records.length} collaborator records from data file`)
